...
export const parseJornadaDiariaProducao = (valor: unknown) => {
  const bruto = texto(valor);
  if (!bruto) return JORNADA_PADRAO_MINUTOS;

  const horario = /^(\d{1,2}):(\d{2})(?::\d{2})?$/.exec(bruto);
  if (horario) {
    const horas = Number(horario[1]);
    const minutos = Number(horario[2]);
    const total = horas * 60 + minutos;
    if (minutos > 59 || total < 1 || total > 1440) {
      throw new Error(`Jornada diária inválida: ${bruto}`);
    }
    return total;
  }

  const horasDecimais = Number(bruto.replace(',', '.'));
  if (Number.isFinite(horasDecimais) && horasDecimais > 0 && horasDecimais <= 24) {
    return Math.round(horasDecimais * 60);
  }

  throw new Error(`Jornada diária inválida: ${bruto}. Use HH:MM, por exemplo 08:00.`);
};
  ...
  lerLinhas(abaEquipe).forEach((linha, indice) => {
    ...
    try {
      membros.push({
        ...
        jornada_diaria_minutos: parseJornadaDiariaProducao(
          valorColuna(linha, [
            'Jornada diária',
            'Jornada diaria',
            'Jornada',
  ...
  XLSX.utils.book_append_sheet(
    ...
    criarPlanilhaModelo(
      [
        ...
        {
          Orientação: 'Equipe de Produção',
          Detalhe:
            'Nome é obrigatório. Jornada diária aceita HH:MM e, quando vazia, assume 08:00.',
        },
        {
          Orientação: 'Tarefas de Produção',
          Detalhe: 'Nome é obrigatório. Categoria é opcional.',
        },
        {
          Orientação: 'Duplicidades',
          Detalhe:
            'Nomes repetidos na planilha ou já cadastrados serão ignorados.',
        },
        {
          Orientação: 'Estoque',
          Detalhe:
            'A importação cria somente cadastros da Produção. Não solicita, reserva nem movimenta estoque.',
        },
      ],
      [32, 92],
    ),
