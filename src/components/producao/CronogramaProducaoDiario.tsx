...
type EscalaGantt = 'quinzena' | 'semana' | 'mes';

const CONFIG_ESCALA: Record<EscalaGantt, { pixelsPorDia: number; quantidadeDias?: number }> = {
  quinzena: { pixelsPorDia: 62, quantidadeDias: 14 },
  semana: { pixelsPorDia: 108, quantidadeDias: 7 },
  mes: { pixelsPorDia: 46 },
};
...
const criarDias = (inicio: Date, fim: Date) => {
  const total = differenceInCalendarDays(fim, inicio) + 1;
  return Array.from({ length: total }, (_, indice) => addDays(inicio, indice));
};

export const CronogramaProducaoDiario = () => {
  const {
    etapas,
    configuracao,
    alertas,
  ...
  useEffect(() => {
    void listarCronograma().catch(() => undefined);
  }, [listarCronograma]);

  useEffect(() => {
    if (configuracao) setConfigForm(configuracao);
  }, [configuracao]);
  ...
  const periodo = useMemo(() => {
    ...
    if (escala === 'mes') {
      ...
      return { inicio, fim: endOfMonth(referencia) };
    }

    return { inicio: referencia, fim: addDays(referencia, 13) };
  }, [escala, referencia]);

  const dias = useMemo(() => criarDias(periodo.inicio, periodo.fim), [periodo]);
  const pixelsPorDia = CONFIG_ESCALA[escala].pixelsPorDia;
  const larguraTimeline = dias.length * pixelsPorDia;
  ...
  const hojeNaFaixa = hojeOffset >= 0 && hojeOffset < larguraTimeline;

  const tituloPeriodo = useMemo(() => {
    if (escala === 'semana') {
      return `Semana de ${format(periodo.inicio, 'dd/MM')} a ${format(periodo.fim, 'dd/MM/yyyy')}`;
    }
    if (escala === 'mes') {
  ...
  const navegar = (direcao: -1 | 1) => {
    setReferencia((atual) => {
      if (escala === 'mes') return addMonths(atual, direcao);
      if (escala === 'semana') return addDays(atual, direcao * 7);
      return addDays(atual, direcao * 14);
    });
  };

  const alterarEscala = (valor: string) => {
    setEscala(valor as EscalaGantt);
    ...
    <div className="space-y-4">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => setConfigAberta(true)}>
            <Settings2 className="mr-2 h-4 w-4" />Capacidade e calendário
          </Button>
          <Button onClick={() => void recalcularCronograma()} disabled={recalculando}>
      ...
      {erro && (
        <Alert variant="destructive">
          ...
          <AlertDescription>{erro}. Confirme se as migrations do cronograma foram aplicadas.</AlertDescription>
        </Alert>
      )}
      ...
      {alertasAltos > 0 && (
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            O cronograma possui {alertasAltos} alerta(s) crítico(s). Verifique etapas sem parâmetros, prazo ultrapassado ou quantidade não alocada.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-4 print:hidden">
        <Card className="p-4">
          <p className="text-xs text-muted-foreground">Equipe disponível/dia</p>
          <p className="text-2xl font-bold">{configuracao?.equipe_disponivel_por_dia ?? '—'}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-muted-foreground">Etapas no cronograma</p>
          <p className="text-2xl font-bold">{etapas.length}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-muted-foreground">Alertas críticos</p>
          <p className={cn('text-2xl font-bold', alertasAltos > 0 && 'text-destructive')}>{alertasAltos}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-muted-foreground">Calendário</p>
          <p className="text-sm font-semibold">
            Sáb: {configuracao?.trabalha_sabado ? 'Sim' : 'Não'} · Dom: {configuracao?.trabalha_domingo ? 'Sim' : 'Não'}
          </p>
        </Card>
      </div>
      ...
      <Tabs defaultValue="gantt">
        ...
        <TabsContent value="gantt" className="mt-4">
          <Card className="overflow-hidden">
            <div className="flex flex-wrap items-center gap-2 border-b p-3 print:hidden">
              ...
              <div className="relative min-w-[240px] flex-1">
                ...
                <Input
                  ...
                  placeholder="Buscar etapa, grupo, projeto ou cidade"
                />
              </div>

              <Select value={escala} onValueChange={alterarEscala}>
              ...
              <Button variant="outline" onClick={() => setReferencia(new Date())}>
            ...
            <div className="border-b bg-muted/20 px-3 py-2 text-sm font-medium capitalize">
              {tituloPeriodo}
            </div>

            <div className="max-h-[70vh] overflow-auto">
              <div className="flex" style={{ width: LABEL_WIDTH + larguraTimeline }}>
                <div className="sticky left-0 z-20 shrink-0 border-r bg-card" style={{ width: LABEL_WIDTH }}>
                  <div className="flex h-16 items-end border-b bg-muted/50 px-3 pb-2 text-xs font-semibold">
                    Projeto / etapa
                  </div>
                  {filtradas.map((etapa) => (
                    <div
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="shrink-0 text-[9px] text-muted-foreground">
                          {statusLabel[etapa.status] ?? etapa.status}
                        </span>
                      </div>
                      <span className="truncate text-[10px] text-muted-foreground">
                        {etapa.projeto_nome}{etapa.grupo_cronograma ? ` · ${etapa.grupo_cronograma}` : ''}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="relative" style={{ width: larguraTimeline }}>
                  <div className="sticky top-0 z-10 flex h-16 border-b bg-muted/50">
                    {dias.map((dia) => (
                      <div
                        ...
                        title={format(dia, "EEEE, dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
                      >
                        <span className="text-[10px] font-semibold capitalize">
                          {format(dia, 'EEE', { locale: ptBR }).replace('.', '')}
                        </span>
                        <span className="text-[10px] text-muted-foreground">{format(dia, 'dd/MM')}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {filtradas.map((etapa) => {
                    ...
                    const inicioVisivel = intervalo && intersecta
                      ? limitarData(intervalo.inicio, periodo.inicio, periodo.fim)
                      : null;
                    const fimVisivel = intervalo && intersecta
                      ? limitarData(intervalo.fim, periodo.inicio, periodo.fim)
                      : null;
                    const esquerda = inicioVisivel
                      ? differenceInCalendarDays(inicioVisivel, periodo.inicio) * pixelsPorDia
                      : 0;
                    const largura = inicioVisivel && fimVisivel
                      ? Math.max(
                        pixelsPorDia,
                        (differenceInCalendarDays(fimVisivel, inicioVisivel) + 1) * pixelsPorDia,
                      )
                      : 0;
                    ...
                    return (
                      <div key={etapa.etapa_id} className="relative border-b" style={{ height: ROW_HEIGHT }}>
                        {dias.map((dia, indice) => (
                          <div
                            ...
                            className={cn(
                              ...
                              (isSaturday(dia) || isSunday(dia)) && 'bg-muted/30',
                            )}
                        ...
                        {intervalo && intersecta && (
                          <div
                            className={cn(
                              'absolute top-3 flex h-8 items-center overflow-hidden rounded text-[10px] font-semibold text-white shadow-sm',
                        ...
                        {!intervalo && (
                          <span className="absolute left-3 top-5 text-[10px] text-muted-foreground">
                            Sem datas de planejamento ou execução
                          </span>
                        )}
                      </div>
                    );
              ...
              {!loading && filtradas.length === 0 && (
                <div className="p-10 text-center text-sm text-muted-foreground">
                  Nenhuma etapa encontrada para os filtros informados.
                </div>
              )}
              {loading && (
                <div className="p-10 text-center text-sm text-muted-foreground">
        ...
        <TabsContent value="plano-diario" className="mt-4">
          <PlanoDiarioProducao />
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={configAberta} onOpenChange={setConfigAberta}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Capacidade e calendário</DialogTitle>
            <DialogDescription>
              Esses parâmetros distribuem automaticamente as etapas nos dias disponíveis.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 pt-3">
            <div className="space-y-2">
              <Label>Equipe disponível por dia</Label>
              <Input
                type="number"
                min="0"
                step="0.5"
                value={configForm.equipe_disponivel_por_dia}
                onChange={(event) => setConfigForm((atual) => ({
                  ...atual,
                  equipe_disponivel_por_dia: Number(event.target.value),
                }))}
              />
            </div>
            <div className="space-y-2">
              <Label>Horizonte de cálculo em dias</Label>
              <Input
                type="number"
                min="30"
                max="1825"
                value={configForm.horizonte_dias}
                onChange={(event) => setConfigForm((atual) => ({
                  ...atual,
                  horizonte_dias: Number(event.target.value),
                }))}
              />
            </div>
            <label className="flex items-center gap-3 rounded-lg border p-3">
              <Checkbox
                checked={configForm.trabalha_sabado}
                onCheckedChange={(checked) => setConfigForm((atual) => ({
                  ...atual,
                  trabalha_sabado: checked === true,
                }))}
              />
              <span className="text-sm">Planejar produção aos sábados</span>
            </label>
            <label className="flex items-center gap-3 rounded-lg border p-3">
              <Checkbox
                checked={configForm.trabalha_domingo}
                onCheckedChange={(checked) => setConfigForm((atual) => ({
                  ...atual,
                  trabalha_domingo: checked === true,
                }))}
              />
              <span className="text-sm">Planejar produção aos domingos</span>
            </label>
            <div className="flex justify-end">
              <Button onClick={() => void salvarConfig()} disabled={recalculando}>
                {recalculando && <RefreshCw className="mr-2 h-4 w-4 animate-spin" />}
                Salvar e recalcular
              </Button>
            </div>
          </div>
