...
const formatarNumeroOp = (numero: number | null | undefined) =>
  numero ? `OP ${String(numero).padStart(6, '0')}` : 'OP pendente';
...
export const HistoricoApontamentosProducaoV2 = ({
  ...
  const imprimir = async (apontamento: ProducaoApontamento) => {
    if (!apontamento.numero_op) {
      toast.error('Este apontamento ainda não possui número de OP. Aplique a migration de numeração.');
      return;
    }

    setImprimindoId(apontamento.id);
    try {
      const processo = apontamento.processo_id ? processosPorId[apontamento.processo_id] : null;
      const localId = apontamento.projeto_local_id ?? processo?.projeto?.local_utilizacao_id ?? null;
      const projeto = localId ? projetosPorLocal[localId] : null;
      const anexos = anexosPorApontamento[apontamento.id] ?? [];
      const fotos = await Promise.all(anexos.map(async (anexo) => ({
      ...
      imprimirOrdemProducao({
        apontamento,
        projetoNome: projeto?.nome ?? (localId ? locaisPorId[localId] : null) ?? 'Projeto não identificado',
        projetoCidade: projeto?.cidade,
        projetoUf: projeto?.uf,
        localOperacional: apontamento.local_tipo,
        processoCodigo: processo?.codigo,
        processoNome: processo?.nome,
        tarefaNome: tarefasPorId[apontamento.tarefa_id] ?? 'Atividade não identificada',
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Histórico, OPs e rastreabilidade</CardTitle>
        <CardDescription>
          Consulte os registros produtivos, responsáveis, status, equipe e evidências. Cada apontamento gera uma OP sequencial.
        </CardDescription>
      </CardHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Projeto / cidade</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={10} className="h-28 text-center text-muted-foreground">Nenhum apontamento encontrado.</TableCell></TableRow>
                ...
                const anexos = anexosPorApontamento[apontamento.id] ?? [];
                const imprimindo = imprimindoId === apontamento.id;

                return (
                  <TableRow key={apontamento.id}>
                    <TableCell className="whitespace-nowrap font-semibold">{formatarNumeroOp(apontamento.numero_op)}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex justify-end gap-1">
                        ...
                        <Button size="icon" variant="ghost" title="Imprimir OP / salvar em PDF" disabled={imprimindo} onClick={() => void imprimir(apontamento)}>
              ...
              {(anexosPorApontamento[galeria.id] ?? []).map((anexo) => (
                <div key={anexo.id} className="rounded-lg border p-3">
                  <div className="mb-2 flex aspect-video items-center justify-center overflow-hidden rounded-md bg-muted/20">
                    {urls[anexo.id] ? <img src={urls[anexo.id]} alt={anexo.file_name} className="h-full w-full object-cover" /> : <ImageIcon className="h-8 w-8 text-muted-foreground" />}
                  </div>
      ...
      <Dialog open={Boolean(detalhes)} onOpenChange={(open) => !open && setDetalhes(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>{detalhes ? formatarNumeroOp(detalhes.numero_op) : 'Rastreabilidade do apontamento'}</DialogTitle><DialogDescription>Responsáveis, tempos e horários registrados pelo sistema.</DialogDescription></DialogHeader>
          {detalhes && (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {detalhes.motivo_improdutivo && <p className="sm:col-span-2"><strong>Motivo improdutivo:</strong> {detalhes.motivo_improdutivo}</p>}
              ...
              <div className="sm:col-span-2 flex justify-end">
                <Button onClick={() => void imprimir(detalhes)} disabled={imprimindoId === detalhes.id}>
                  {imprimindoId === detalhes.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Printer className="mr-2 h-4 w-4" />}
                  Imprimir OP / PDF
                </Button>
              </div>
            </div>
