...
const jornadaParaMinutos = (valor: string): number | null => {
  const texto = valor.trim();
  if (!texto) return null;
  const correspondencia = /^(\d{1,2}):(\d{2})$/.exec(texto);
  if (!correspondencia) throw new Error('Informe a jornada no formato HH:MM.');
  const horas = Number(correspondencia[1]);
  const minutos = Number(correspondencia[2]);
  const total = horas * 60 + minutos;
  if (minutos > 59 || total < 1 || total > 1440) {
    throw new Error('A jornada deve estar entre 00:01 e 24:00.');
  }
  return total;
};
...
export const ConfiguracoesProducao = ({
  ...
  const [jornadaEdicao, setJornadaEdicao] = useState('');
  ...
  const cadastrarMembro = async (event: FormEvent) => {
    ...
    try {
      await criarMembro(
        ...
        jornadaParaMinutos(jornadaMembro),
      );
      setNomeMembro('');
      setApelidoMembro('');
      setFuncaoMembro('');
      setValorHoraMembro('');
      setJornadaMembro('08:00');
      toast.success('Membro cadastrado com jornada diária.');
    } catch (error) {
  ...
  const salvarEdicao = async (event: FormEvent) => {
    ...
    try {
      await editarMembro(membroEditando.id, {
        ...
        jornada_diaria_minutos: jornadaParaMinutos(jornadaEdicao),
      });
      setMembroEditando(null);
      toast.success('Membro e jornada atualizados.');
    } catch (error) {
