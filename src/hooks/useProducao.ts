...
const normalizarTempos = (duracao: number, produtivos?: number | null, improdutivos?: number | null, motivo?: string | null) => {
  const minutosImprodutivos = Number(improdutivos ?? 0);
  const minutosProdutivos = produtivos ?? duracao - minutosImprodutivos;
  if (!Number.isInteger(minutosProdutivos) || !Number.isInteger(minutosImprodutivos)) throw new Error('Os tempos devem ser minutos inteiros.');
  if (minutosProdutivos < 0 || minutosImprodutivos < 0 || minutosProdutivos + minutosImprodutivos !== duracao) throw new Error('A soma dos tempos deve ser igual à duração.');
  if (minutosImprodutivos > 0 && !motivo?.trim()) throw new Error('Informe o motivo do tempo improdutivo.');
  return { minutos_produtivos: minutosProdutivos, minutos_improdutivos: minutosImprodutivos, motivo_improdutivo: minutosImprodutivos > 0 ? motivo!.trim() : null };
};
...
export const useProducao = () => {
  ...
  const listarTarefas = useCallback(async (somenteAtivas = true) => {
    let consulta = db.from('producao_tarefas').select('*').order('nome');
  ...
  const criarTarefa = useCallback(async (nome: string, categoria?: string | null) => {
    const { data: id, error } = await db.rpc('criar_tarefa_producao', { p_nome: nome, p_categoria: categoria ?? null });
    if (error) throw error;
    const { data, error: readError } = await db.from('producao_tarefas').select('*').eq('id', id).single();
  ...
  const listarMembrosProducao = useCallback(async (somenteAtivos = true) => {
    let consulta = db.from('producao_membros').select('*').order('nome');
  ...
  const salvarMembro = useCallback(async (id: string | null, dados: NovoMembroProducao) => {
    const { data: membroId, error } = await db.rpc('salvar_membro_producao', {
      p_id: id, p_nome: dados.nome, p_apelido: dados.apelido ?? null, p_funcao: dados.funcao ?? null,
      p_valor_hora: dados.valor_hora ?? null, p_ativo: dados.ativo ?? true,
    });
    if (error) throw error;
    const { data, error: readError } = await db.from('producao_membros').select('*').eq('id', membroId).single();
    if (readError) throw readError;
    return data as ProducaoMembro;
  }, []);

  const criarMembroProducao = useCallback(async (nome: string, apelido?: string | null, funcao?: string | null, valorHora?: number | null) => {
  ...
  const listarApontamentos = useCallback(async (filtros: FiltrosProducao = {}) => {
    setLoading(true);
    try {
      let consulta = db.from('producao_apontamentos').select('*').order('data', { ascending: false }).order('inicio', { ascending: false });
      ...
      if (filtros.projeto_id) consulta = consulta.eq('projeto_id', filtros.projeto_id);
      if (filtros.projeto_local_operacional_id) consulta = consulta.eq('projeto_local_operacional_id', filtros.projeto_local_operacional_id);
  ...
  const executarApontamentoRpc = useCallback(async (rpc: 'criar_apontamento_producao_operacional' | 'editar_apontamento_producao_operacional', novo: NovoApontamentoProducao, apontamentoId?: string) => {
    if (!novo.processo_id && (!novo.projeto_id || !novo.projeto_local_operacional_id)) throw new Error('Selecione uma etapa ou informe projeto e local operacional.');
    ...
    const parametros = {
      ...
      p_projeto_id: novo.projeto_id ?? null,
      p_projeto_local_operacional_id: novo.projeto_local_operacional_id ?? null,
  ...
  const criarApontamento = useCallback(async (novo: NovoApontamentoProducao) => {
    const id = await executarApontamentoRpc('criar_apontamento_producao_operacional', novo);
    const { data, error } = await db.from('producao_apontamentos').select('*').eq('id', id).single();
    if (error) throw error;
  ...
  const editarApontamento = useCallback(async (id: string, alteracoes: AtualizacaoApontamento, membrosIds?: string[]) => {
    ...
    const novo: NovoApontamentoProducao = {
      ...
      projeto_id: alteracoes.projeto_id === undefined ? atual.projeto_id : alteracoes.projeto_id,
      projeto_local_operacional_id: alteracoes.projeto_local_operacional_id === undefined ? atual.projeto_local_operacional_id : alteracoes.projeto_local_operacional_id,
    ...
    await executarApontamentoRpc('editar_apontamento_producao_operacional', novo, id);
    const { data, error } = await db.from('producao_apontamentos').select('*').eq('id', id).single();
    if (error) throw error;
    const apontamento = data as ProducaoApontamento;
  ...
  const cancelarApontamento = useCallback(async (id: string, justificativa = 'Cancelado pelo usuário') => {
    const { error } = await db.rpc('cancelar_apontamento_producao', { p_apontamento_id: id, p_justificativa: justificativa });
    if (error) throw error;
    const { data, error: readError } = await db.from('producao_apontamentos').select('*').eq('id', id).single();
    if (readError) throw readError;
    const apontamento = data as ProducaoApontamento;
    setApontamentos((atuais) => atuais.map((item) => item.id === id ? apontamento : item));
    return apontamento;
  ...
  const conferirApontamento = useCallback(async (id: string) => {
    const { error } = await db.rpc('conferir_apontamento_producao', { p_apontamento_id: id });
    if (error) throw error;
    const { data, error: readError } = await db.from('producao_apontamentos').select('*').eq('id', id).single();
    if (readError) throw readError;
    const apontamento = data as ProducaoApontamento;
  ...
  const listarMembros = useCallback(async (apontamentoId: string) => {
    const { data, error } = await db.from('producao_apontamento_membros').select('*').eq('apontamento_id', apontamentoId).order('nome_snapshot');
    if (error) throw error;
  ...
  return { tarefas, membrosProducao, apontamentos, loading, listarTarefas, criarTarefa, listarMembrosProducao, criarMembroProducao, editarMembroProducao, inativarMembroProducao, listarApontamentos, criarApontamento, editarApontamento, cancelarApontamento, conferirApontamento, listarMembros, salvarMembros };
};
