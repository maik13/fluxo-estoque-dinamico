...
-- Fluxo oficial: Projeto -> Etapa -> OP -> Apontamentos.
-- Apontamentos históricos permanecem como avulsos quando não possuírem OP.

BEGIN;

-- Remove a modelagem experimental que numerava o apontamento como se fosse uma OP.
...
CREATE TABLE IF NOT EXISTS public.producao_ordens_producao (
  ...
  status TEXT NOT NULL DEFAULT 'liberada'
    CHECK (status IN ('rascunho','liberada','em_execucao','concluida','cancelada')),
  ...
  CONSTRAINT producao_ordem_datas_validas
...
DROP POLICY IF EXISTS producao_ordens_leitura_atual
...
DROP POLICY IF EXISTS producao_ordem_eventos_leitura_atual
  ON public.producao_ordem_eventos;
...
AS $$
  SELECT COALESCE(
    (SELECT NULLIF(BTRIM(p.nome), '')
      ...
      LIMIT 1),
    (SELECT NULLIF(BTRIM(u.raw_user_meta_data->>'name'), '')
       FROM auth.users u
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(a.quantidade_produzida)
      FILTER (WHERE a.status = 'conferido'), 0)
  ...
  CASE p_acao
    ...
    WHEN 'concluir' THEN
      ...
      IF v_realizado < v_op.quantidade_planejada
        ...
        RAISE EXCEPTION
          'A quantidade realizada é menor que a planejada. Informe uma justificativa';
      END IF;
      v_novo := 'concluida';
...
CREATE OR REPLACE FUNCTION public.atualizar_status_ordem_producao(
  p_ordem_id UUID
)
RETURNS VOID
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT FOUND OR v_op.status = 'cancelada' THEN
    RETURN;
  END IF;
  ...
  SELECT
    ...
    COALESCE(SUM(quantidade_produzida)
      FILTER (WHERE status = 'conferido'), 0),
  ...
  UPDATE public.producao_ordens_producao
     SET status = CASE
           WHEN v_realizado >= quantidade_planejada
                AND v_pendentes = 0
                AND v_ativos > 0 THEN 'concluida'
           WHEN v_ativos > 0 THEN 'em_execucao'
           WHEN status = 'concluida' THEN 'concluida'
           ELSE 'liberada'
         END,
         data_inicio_real = CASE
           WHEN v_ativos > 0 THEN COALESCE(data_inicio_real, v_primeira)
           ELSE data_inicio_real
         END,
         data_fim_real = CASE
           WHEN v_realizado >= quantidade_planejada
                AND v_pendentes = 0
                AND v_ativos > 0 THEN COALESCE(v_ultima, CURRENT_DATE)
           WHEN status <> 'concluida' THEN NULL
           ELSE data_fim_real
         END,
         updated_at = NOW()
   WHERE id = p_ordem_id;
...
BEGIN
  ...
  IF TG_OP = 'UPDATE'
     AND OLD.ordem_producao_id IS DISTINCT FROM NEW.ordem_producao_id THEN
    PERFORM public.atualizar_status_ordem_producao(OLD.ordem_producao_id);
  END IF;

  PERFORM public.atualizar_status_ordem_producao(NEW.ordem_producao_id);
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_apontamento_atualiza_ordem
  ON public.producao_apontamentos;
CREATE TRIGGER trg_apontamento_atualiza_ordem
AFTER INSERT OR UPDATE OR DELETE
ON public.producao_apontamentos
FOR EACH ROW
EXECUTE FUNCTION public.trg_atualizar_status_ordem_producao();

CREATE OR REPLACE FUNCTION public.validar_encerramento_etapa_com_ops()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  IF NEW.status IN ('finalizado','cancelado')
     AND OLD.status IS DISTINCT FROM NEW.status
     AND EXISTS (
       SELECT 1
       FROM public.producao_ordens_producao o
       WHERE o.processo_id = NEW.id
         AND o.status IN ('rascunho','liberada','em_execucao')
     ) THEN
    RAISE EXCEPTION
      'A Etapa possui Ordens de Produção abertas. Conclua ou cancele todas as OPs antes de encerrar a Etapa';
  END IF;
...
DROP TRIGGER IF EXISTS trg_validar_encerramento_etapa_com_ops
  ON public.producao_processos;
CREATE TRIGGER trg_validar_encerramento_etapa_com_ops
BEFORE UPDATE OF status
ON public.producao_processos
FOR EACH ROW
EXECUTE FUNCTION public.validar_encerramento_etapa_com_ops();

CREATE OR REPLACE FUNCTION public.bloquear_exclusao_etapa_com_ops()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.producao_ordens_producao o
    WHERE o.processo_id = OLD.id
  ) THEN
    RAISE EXCEPTION
      'A Etapa possui Ordens de Produção e não pode ser excluída. Cancele a Etapa para preservar a rastreabilidade';
  END IF;
...
DROP TRIGGER IF EXISTS trg_bloquear_exclusao_etapa_com_ops
  ON public.producao_processos;
CREATE TRIGGER trg_bloquear_exclusao_etapa_com_ops
BEFORE DELETE
ON public.producao_processos
FOR EACH ROW
EXECUTE FUNCTION public.bloquear_exclusao_etapa_com_ops();
  ...
  SELECT
    ...
    CASE
      WHEN o.quantidade_planejada > 0 THEN
        LEAST(100, ROUND(
          (COALESCE(a.realizado,0) / o.quantidade_planejada) * 100,
          2
        ))
      ELSE 0
    END,
    ...
    o.updated_at
  FROM public.producao_ordens_producao o
  JOIN public.producao_processos p
    ON p.id = o.processo_id
  JOIN public.producao_projetos pr
    ON pr.id = o.projeto_id
  LEFT JOIN (
    SELECT
      ordem_producao_id,
...
BEGIN
  ...
  IF COALESCE(p_minutos_produtivos,0)
     + COALESCE(p_minutos_improdutivos,0) <> p_duracao_minutos THEN
  ...
  IF p_ordem_producao_id IS NOT NULL THEN
    ...
    IF p_quantidade_produzida IS NOT NULL
       AND v_quantidade_aberta + p_quantidade_produzida
           > v_op.quantidade_planejada THEN
      RAISE EXCEPTION
    ...
    IF p_processo_id IS NOT NULL
       AND p_processo_id <> v_op.processo_id THEN
    ...
    UPDATE public.producao_processos
       SET status = CASE
           ...
           END,
           data_inicio_real = CASE
             WHEN status = 'planejado' THEN COALESCE(data_inicio_real, p_data)
             ELSE data_inicio_real
           END,
           updated_at = NOW()
     WHERE id = v_op.processo_id;
  ...
  ELSE
    IF p_processo_id IS NOT NULL OR p_projeto_local_id IS NULL THEN
      RAISE EXCEPTION
        'Apontamento planejado exige uma OP. Para atividade avulsa, informe somente o projeto/local';
    END IF;
  ...
  LOOP
    INSERT INTO public.producao_apontamento_membros (
      ...
      nome_snapshot,
      valor_hora_snapshot,
      jornada_diaria_minutos_snapshot,
      minutos_produtivos_snapshot,
      ...
      v_membro.nome,
      v_membro.valor_hora,
      v_membro.jornada_diaria_minutos,
      p_minutos_produtivos,
      p_minutos_improdutivos
    );
  END LOOP;
...
AS $$
  ...
  LEFT JOIN (
    SELECT
      processo_id,
      COALESCE(SUM(quantidade_produzida),0) AS realizado
    FROM public.producao_apontamentos
    WHERE status = 'conferido'
...
REVOKE ALL ON FUNCTION public.transicao_ordem_producao(UUID,TEXT,TEXT)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.atualizar_status_ordem_producao(UUID)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.listar_ordens_producao(UUID,TEXT)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.criar_apontamento_producao(
  DATE,UUID,UUID,UUID,UUID,TEXT,NUMERIC,TIME,TIME,
  INTEGER,INTEGER,INTEGER,TEXT,TEXT,UUID[]
) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.listar_gantt_producao()
  FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.transicao_ordem_producao(UUID,TEXT,TEXT)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.listar_ordens_producao(UUID,TEXT)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.criar_apontamento_producao(
  DATE,UUID,UUID,UUID,UUID,TEXT,NUMERIC,TIME,TIME,
  INTEGER,INTEGER,INTEGER,TEXT,TEXT,UUID[]
) TO authenticated;
GRANT EXECUTE ON FUNCTION public.listar_gantt_producao()
  TO authenticated;
