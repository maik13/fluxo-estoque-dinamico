-- Consolidação final da hierarquia Projeto -> Etapa -> OP.
-- Idempotente e segura para bancos que receberam zero, uma ou mais versões anteriores.
...
-- Remove as duas assinaturas que chegaram a existir durante a evolução da feature.
...
CREATE FUNCTION public.configurar_projeto_producao_v2(
...
BEGIN
  ...
  IF COALESCE(v_total_validas, 0) = 0 THEN
    v_status := 'planejado';
    v_inicio_real := NULL;
    v_fim_real := NULL;
...
-- Remove nomes de trigger utilizados nas versões anteriores.
...
DROP TRIGGER IF EXISTS trg_producao_processos_sincronizar_projeto
  ON public.producao_processos;
DROP TRIGGER IF EXISTS trg_sincronizar_status_projeto_producao
  ON public.producao_processos;

CREATE TRIGGER trg_sincronizar_status_projeto_producao
AFTER INSERT OR DELETE OR UPDATE OF status, data_inicio_real, data_fim_real, finalizado_em, cancelado_em, projeto_id
ON public.producao_processos
FOR EACH ROW
