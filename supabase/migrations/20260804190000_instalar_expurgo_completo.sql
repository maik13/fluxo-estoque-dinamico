-- Instalação completa e idempotente do expurgo administrativo de Etapas de teste.
-- Cria primeiro o helper administrativo e depois as RPCs esperadas pelo frontend.
...
BEGIN
  ...
  IF to_regprocedure('public.recalcular_cronograma_producao_interno(uuid,text)') IS NOT NULL THEN
    EXECUTE 'SELECT public.recalcular_cronograma_producao_interno($1, $2)'
      USING v_user, COALESCE(v_nome_usuario, 'Administrador');
  END IF;
END;
$$;
