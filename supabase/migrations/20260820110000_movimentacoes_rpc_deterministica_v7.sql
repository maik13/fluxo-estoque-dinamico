...
-- Função única de autorização, compatível com o modelo novo e o legado.
...
BEGIN
  ...
  IF p_local_utilizacao_id IS NOT NULL
     AND NOT EXISTS (
       SELECT 1
  ...
  -- Auditoria é complementar. A edição não depende da existência do log.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'A movimentação não foi excluída';
  END IF;
