-- Cards hierárquicos de Produção: período previsto e ciclo automático do Projeto.
-- Projeto = Planejado -> Em andamento -> Concluído conforme suas Etapas.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'producao_projetos_periodo_previsto_check'
      AND conrelid = 'public.producao_projetos'::regclass
  ) THEN
    ALTER TABLE public.producao_projetos
      ADD CONSTRAINT producao_projetos_periodo_previsto_check
      CHECK (
...
BEGIN
  ...
  SELECT
    COUNT(*) FILTER (WHERE pp.status <> 'cancelado'),
    COUNT(*) FILTER (WHERE pp.status = 'finalizado'),
    COUNT(*) FILTER (
      WHERE pp.status <> 'cancelado'
        AND pp.status IN ('em_andamento', 'pausado', 'bloqueado', 'finalizado')
    ),
    MIN(pp.data_inicio_real) FILTER (WHERE pp.status <> 'cancelado'),
    MAX(pp.data_fim_real) FILTER (WHERE pp.status = 'finalizado')
  INTO
    v_total_validas,
    v_finalizadas,
    v_iniciadas,
    v_inicio_real,
    v_fim_real
  FROM public.producao_processos pp
  ...
  IF v_total_validas = 0 THEN
    ...
    v_inicio_real := NULL;
    v_fim_real := NULL;
  ELSIF v_finalizadas = v_total_validas THEN
    v_status := 'concluido';
  ...
  SET status = v_status,
      ...
      data_fim_real = CASE WHEN v_status = 'concluido' THEN v_fim_real ELSE NULL END,
      updated_at = NOW()
  WHERE id = p_projeto_id
    AND (
      status IS DISTINCT FROM v_status
      OR data_inicio_real IS DISTINCT FROM v_inicio_real
      OR data_fim_real IS DISTINCT FROM CASE WHEN v_status = 'concluido' THEN v_fim_real ELSE NULL END
    );
END;
$$;

REVOKE ALL ON FUNCTION public.recalcular_status_projeto_producao(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.recalcular_status_projeto_producao(UUID) TO authenticated;
...
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id FROM public.producao_projetos LOOP
    PERFORM public.recalcular_status_projeto_producao(r.id);
  END LOOP;
END $$;
