-- Corrige somente os nomes e descrições exibidos no painel de permissões.
-- As chaves, campos e exceções já salvas permanecem inalterados:
-- pode_solicitar_material = Retirada de Material (cartão verde)
-- pode_solicitacao_material = Solicitação de Material (cartão laranja)
